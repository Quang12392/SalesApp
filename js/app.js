...
const App = {
  ...
  reportProducts(el, orders, view) {
    ...
    const topRevHtml = br.map(([n,v],i) => `<div class="kvt-row"><div class="kvt-bar" style="width:${Math.round(v/mr*100)}%;background:rgba(26,115,232,0.1)"></div><span class="kvt-rank">${i+1}</span><span class="kvt-name">${n}</span><span class="kvt-val blue">${fmtd(v)}</span></div>`).join('');
    const topQtyHtml = bq.map(([n,v],i) => `<div class="kvt-row"><div class="kvt-bar" style="width:${Math.round(v/mq*100)}%;background:rgba(16,185,129,0.1)"></div><span class="kvt-rank">${i+1}</span><span class="kvt-name">${n}</span><span class="kvt-val green">${v}</span></div>`).join('');
    el.innerHTML=`<h2 class="report-title">B\u00e1o c\u00e1o h\u00e0ng h\u00f3a</h2>${cards}<div class="card" style="margin-top:16px"><div class="card-header"><h3>Top h\u00e0ng theo doanh thu</h3></div><div class="card-body" style="padding:0">${topRevHtml || '<div style="text-align:center;padding:24px;color:#9CA3AF">Kh\u00f4ng c\u00f3 d\u1eef li\u1ec7u</div>'}</div></div><div class="card" style="margin-top:16px"><div class="card-header"><h3>Top h\u00e0ng theo s\u1ed1 l\u01b0\u1ee3ng</h3></div><div class="card-body" style="padding:0">${topQtyHtml || '<div style="text-align:center;padding:24px;color:#9CA3AF">Kh\u00f4ng c\u00f3 d\u1eef li\u1ec7u</div>'}</div></div>`;
  },
