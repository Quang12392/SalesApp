...
const App = {
  ...
  reportProducts(el, orders, view) {
    ...
    const topRevHtml = br.map(([n,v],i) => `<div class="kvt-row"><span class="kvt-rank">${i+1}</span><span class="kvt-name">${n}</span><span class="kvt-val blue">${fmtd(v)}</span></div>`).join('');
    const topQtyHtml = bq.map(([n,v],i) => `<div class="kvt-row"><span class="kvt-rank">${i+1}</span><span class="kvt-name">${n}</span><span class="kvt-val green">${v}</span></div>`).join('');
