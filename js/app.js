...
const App = {
        ...
        return `<div class="product-mobile-card" data-pid="${p.id}">
          ...
          <div class="pmc-row3">
            <span class="pmc-actions">
              <button class="btn-icon edit-p-m" data-id="${p.id}" title="Sửa">✏️</button>
              <button class="btn-icon danger del-p-m" data-id="${p.id}" title="Xóa">🗑️</button>
            </span>
          </div>
        </div>`;
  ...
  reportSales(el, orders, view) {
    ...
    if(view==='chart'){
      ...
      this.rptPieChart('rc5',['Tiền mặt','Chuyển khoản','Ship COD','Ship Thường'],[cS,bS,codS,shipS],['#2E7D32','#ff5cc0','#F59E0B','#8B5CF6']);
      ...
      el.innerHTML=`<h2 class="report-title">Báo cáo bán hàng</h2>${cards}<div class="card" style="margin-top:16px"><div class="card-header"><h3>Phân loại thanh toán</h3></div><div class="card-body"><div class="fin-summary"><div class="fin-row" style="font-weight:600;color:var(--text-secondary);font-size:0.8rem"><span>Hình thức</span><span>Số đơn · Số tiền</span></div><div class="fin-row"><span>💵 Tiền mặt</span><span>${cT.length} đơn · <b>${fmtd(cS)}</b></span></div><div class="fin-row"><span>🏦 Chuyển khoản</span><span>${bT.length} đơn · <b>${fmtd(bS)}</b></span></div><div class="fin-row"><span>🚚 Ship COD</span><span>${codT.length} đơn · <b>${fmtd(codS)}</b></span></div><div class="fin-row"><span>📦 Ship Thường</span><span>${shipT.length} đơn · <b>${fmtd(shipS)}</b></span></div><div class="fin-row fin-total"><span>Tổng</span><span>${cnt} đơn · ${fmtd(rev)}</span></div></div></div></div><div class="card" style="margin-top:16px"><div class="card-header"><h3>Doanh thu theo nhân viên</h3></div><div class="card-body"><div class="fin-summary">${empSorted.map(([n,d])=>`<div class="fin-row"><span style="font-weight:600">${n}</span><span>${d.c} đơn · <b>${fmtd(d.r)}</b></span></div>`).join('')}<div class="fin-row fin-total"><span>Tổng</span><span>${cnt} đơn · ${fmtd(rev)}</span></div></div></div></div><div class="card" style="margin-top:16px"><div class="card-header"><h3>Chi tiết ${periodLabel}</h3></div><div class="card-body"><div class="fin-summary">${dr.reverse().map(([d,v])=>`<div class="fin-row"><span>${d}</span><span>${v.c} đơn · <b>${fmtd(v.r)}</b></span></div>`).join('')}<div class="fin-row fin-total"><span>Tổng</span><span>${cnt} đơn · ${fmtd(rev)}</span></div></div></div></div>`;
    }
  },
  ...
  reportFinance(el, orders, view) {
    ...
    if(view==='chart'){
      ...
      this.rptPieChart('rc6',['Giá vốn','Lợi nhuận','Chiết khấu'],[cost,profit,disc],['#EF4444','#2E7D32','#F59E0B']);
    } else {
