...
### Đã xác minh khóa và nối upstream Worker — mốc tiếp tục mới nhất

- Người dùng báo đã thao tác Kết nối Worker. Đã kiểm tra độc lập bằng `provision.ps1 -Mode probe-backend` với URL /exec đang dùng: gửi phong bì ký đúng key và access token probe cố ý không hợp lệ; backend trả `gatewayVersion=1`, `AUTH_REQUIRED`. Như vậy deployment đang chạy có `appGateway` và `KH_GATEWAY_SECRET` khớp. Không yêu cầu dán key/deploy lại cùng bản. Probe chỉ ghi nonce kỹ thuật, không gọi thao tác bán hàng.
- Đã nạp `KH_APPS_SCRIPT_URL` vào Worker bằng URL SalesApp đã xác minh. Lần kiểm tra ngay sau upload còn thấy config cũ do cập nhật chưa lan hết; lần kiểm tra tiếp theo đã xác nhận `Device authorized:true; backendConfigured:true`.
- Đã kiểm tra xuyên Worker → Apps Script bằng login với username ngẫu nhiên không tồn tại, trả HTTP 401 `LOGIN_FAILED`; không dùng password thật, không tạo user/phiên, không migrate password hoặc ghi giao dịch. `/v1/api` với token sai trả HTTP 401 `AUTH_REQUIRED` đúng kỳ vọng.
- **Chưa chuyển client:** PWA vẫn v380, `GATEWAY_ENABLED=false`; chưa xác minh ghép mã ở extension thật và chưa bật `KH_REQUIRE_GATEWAY`/đóng các deployment cũ. Nối upstream không có nghĩa toàn bộ hệ thống đã được bảo vệ.
- Việc tiếp theo: reload extension đúng bản 1.0.2, mở Cài đặt → Kết nối SalesApp bằng mã riêng máy, điền URL Worker và mã `sd1_...` của đúng máy. Chỉ copy mã thiết bị vào clipboard khi người dùng đã mở đúng ô; không nhầm với khóa gateway vừa đặt trên Google. Sau đó chốt thời điểm chuyển PWA khi hai điện thoại không có thao tác thanh toán đang dở, giữ nguyên đơn tạm/hàng chờ, kiểm thử phiên thật và phát hành bản bật gateway đồng nhất trên hai Hosting. Chỉ khóa legacy khi tất cả client đã được kiểm chứng.

### Sửa lưu mã extension 1.0.3 — mốc mới nhất

- 1.0.2 dừng trước khi lưu vì bắt buộc `chrome.storage.local.setAccessLevel`, trong khi hàm không có ở môi trường thực tế; sau mở lại hiện Không đọc được cấu hình máy. Không tự kết luận phải cập nhật Chrome chỉ từ thông báo của extension; không đổi khóa/backend vì lỗi client này.
- Đã thay kho credential bằng IndexedDB `khs_extension_device` của origin tiện ích, chờ transaction commit (readwrite strict) trước khi báo thành công. Chrome local chỉ giữ `salesAppDevicePaired` và URL không bí mật. Cờ đã ghép chặn fallback nếu mất/hỏng kho mã; metadata URL có thể khôi phục từ record chuẩn. Không lưu mã trên trang TikTok hoặc storage sync; cách ly origin không phải mã hóa chống người kiểm soát máy/profile.
- Mã legacy nếu có được chuyển vào IndexedDB rồi mới xóa bản chrome.storage.local sau commit. Lỗi ghi giữ nguyên bản cũ; cấu hình cũ/mới xung đột không tự xóa hoặc chọn đại. Không hạ bảo vệ bằng cách bỏ qua lỗi rồi lưu mã vào vùng không được bảo vệ.
- Giao diện 1.0.3 điền sẵn Worker URL, ghi rõ Đã lưu mã máy và placeholder Mã đã lưu. Ô mã trống sau mở lại không phải mất mã nếu URL/trạng thái đã phục hồi. Cần đóng cửa sổ extension cũ → Reload ở chrome://extensions → kiểm tra 1.0.3 → mở lại; không gỡ/xóa dữ liệu. Lần 1.0.2 chưa lưu được phải dán lại mã hiện có một lần.
- 18 test tự động với fake-indexeddb 6.2.5 đạt, bao gồm thiếu API, mở lại, ghi lỗi/abort, migration, xung đột và không fallback. Fixture loopback dùng IndexedDB thật đã ghi, đóng tab, mở tab mới và khôi phục/gửi đúng mã giả; không có yêu cầu tới Worker/Sheet thật trong các test. Chưa tự động thoát toàn bộ Chrome/profile thật của người dùng, cần họ Reload và xác nhận kết nối thực tế.
- Chỉ sửa module lưu mã, manifest version, test và tài liệu; không sửa bot, Mapping, giá hoặc các thay đổi UI/background có từ trước. Không cần deploy Apps Script/Worker/Firebase. SalesApp vẫn v380, gateway PWA chưa bật; mã máy/gateway secret được giữ nguyên.
- Source extension 1.0.3 đã push `ec9ffdd` lên nhánh `master` của TikTok-Tools; hướng dẫn backend Private đã push `9881a33`. Máy người dùng cần Reload tiện ích rồi ghép lại bằng mã máy chính đã có; chưa coi ghép thực tế thành công chỉ từ kết quả test giả lập/native IndexedDB fixture.
